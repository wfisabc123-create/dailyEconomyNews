...
export async function news(rssQuery, translate = false) {
  const query = rssQuery.includes('when:') ? rssQuery : `${rssQuery} when:1d`;
  let articles = [];

  const clean = (value = '') => value.replace(/<!\[CDATA\[|\]\]>/g, '').replace(/&amp;/g, '&').replace(/&quot;/g, '"').replace(/&#39;/g, "'").trim();

  try {
    const url = `https://news.google.com/rss/search?q=${encodeURIComponent(query)}&hl=ko&gl=KR&ceid=KR:ko&_=${Date.now()}`;
    const response = await fetchFast(url, { 'User-Agent': 'Mozilla/5.0 Economic-Briefing-Dashboard/1.0' }, 6500);
    const xml = await response.text();
    articles = [...xml.matchAll(/<item>([\s\S]*?)<\/item>/g)].map((match) => {
      const item = match[1];
    ...
    console.warn(`Fresh query with when:1d failed: ${error.message}`);
  }

  if (articles.length < 3) {
    try {
      const fallbackUrl = `https://news.google.com/rss/search?q=${encodeURIComponent(rssQuery)}&hl=ko&gl=KR&ceid=KR:ko&_=${Date.now()}`;
      const response = await fetchFast(fallbackUrl, { 'User-Agent': 'Mozilla/5.0 Economic-Briefing-Dashboard/1.0' }, 6500);
      const xml = await response.text();
      articles = [...xml.matchAll(/<item>([\s\S]*?)<\/item>/g)].map((match) => {
        const item = match[1];
        const value = (tag) => clean((item.match(new RegExp(`<${tag}>([\\s\\S]*?)<\\/${tag}>`)) || [])[1] || '');
      ...
      console.warn(`Fallback query failed: ${e.message}`);
    }
  }
  ...
  if (!translate) return topArticles;
  return Promise.all(topArticles.map(async (article) => ({
    ...article,
